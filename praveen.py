...
class AnalysisWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # --- 1. SPACY MACHINE LEARNING EXTRACTION ---
            try:
                # Load the custom AI model you just trained
                nlp = spacy.load("./custom_project_ner")
                doc = nlp(self.text)
                
                # Scan the document for entities it recognizes as a PROJECT_NAME
                for ent in doc.ents:
                    if ent.label_ == "PROJECT_NAME":
                        results["project_name"] = ent.text.title()
                        break # Grab the first one it finds
            except Exception as e:
                print(f"Model not found or error: {e}. Train the model first!")
