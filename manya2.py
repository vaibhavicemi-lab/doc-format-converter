...
class OfflineApp(QMainWindow):
    ...
    def handle_dropdown(self, text, combo):
        if text == "Enter Other...":
            custom_text, ok = QInputDialog.getText(self, "Custom Entry", "Enter your custom option:")
            if ok and custom_text.strip():
                # insert custom_text before "Enter Other..."
                idx = combo.count() - 1
                combo.insertItem(idx, custom_text.strip())
                combo.setCurrentIndex(idx)
            else:
                combo.setCurrentIndex(0) # revert wrapper
    ...
    def init_ui(self):
        ...
        for label, items in dropdowns:
            ...
            combo.currentTextChanged.connect(lambda text, cb=combo: self.handle_dropdown(text, cb))
            row.addWidget(lbl)
            row.addWidget(combo)
